...
\author{Alper Erdoğan -- 22401012\\
Mert Samet Kayacıoğlu -- 21401923}
\date{May 9, 2026}
...
\begin{itemize}
  \item \textbf{GitHub Repository:} \url{https://github.com/AlperErd0gan/INF473_Project}
  \item \textbf{Deployed Application:} \url{https://gsu-advisor.onrender.com/}
\end{itemize}
...
The implemented result screen exposes four agent verdicts: three independent
verifier agents and one master aggregation agent. The parser, report writer,
report critic, and database history tool are supporting pipeline roles around
these four displayed verdicts; they are not counted as independent
graduation-verifier agents.
...
\midrule
\texttt{MasterAgent} &
Verifier verdicts to final \texttt{AgentVerdict}. &
Aggregates the three independent verifier results with strict conjunction,
marks the student eligible only if all three pass, and exposes the final
advisor-style decision on the result screen. \\
\bottomrule
\caption{The four agent verdicts exposed by the implemented system.}
\end{longtable}

Around these displayed agents, \texttt{TranscriptParserAgent} prepares
\texttt{ParsedTranscript}, \texttt{MasterAgent} also handles parsing
orchestration, cross-examination, logging, report generation, and final
formatting, \texttt{MasterReportAgent} writes the advisor-style report,
\texttt{ReportCriticAgent} reviews the report text, and
\texttt{check\_student\_history} is a tool used only during report generation.
